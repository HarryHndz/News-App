...
type TNewProps = {
  chat: IChatList
  handleDetailChat:()=>void
}

export const New = ({chat, handleDetailChat}: TNewProps)=>{
  return(
    <div className="group flex flex-row items-center gap-2 bg-slate-700 p-4 rounded-lg border border-slate-600 py-2 my-2 cursor-pointer hover:bg-slate-600 transition-all duration-300 ease-in-out">
        ...
        <div className="flex flex-col break-words min-w-0">
          <h3 className="text-slate-100 font-bold">{dateReadable(new Date(chat.createdAt))}</h3>
          <p className="text-slate-300 truncate" title={chat.title}>{chat.title}</p>
        </div>
      <button
        onClick={handleDetailChat} 
        className="bg-indigo-600 text-white p-2 rounded-full hover:bg-indigo-700 transition-all duration-300 ease-in-out cursor-pointer invisible group-hover:visible">
